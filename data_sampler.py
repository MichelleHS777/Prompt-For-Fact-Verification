...
while len(fake_id) != 4000:
    ...
    if random_sample[0]['claimId'] in fake_id:
        continue
    if len(random_sample[1]['evidences']) == 0:
        continue

    claimId = random_sample[0]['claimId']
    claim = random_sample[0]['claim']
    evidence = random_sample[1]['evidences']
    fake_id.append(claimId) # count the collected numbers of data

    # write claim + "" in json
    # if len(fake_id) <= 3200:
    fake = {'claimId': claimId, 'claim': claim, 'evidences': "", 'label': 2}
    # write claim+non-evidence in json
    # else:
    # fake = {'claimId': claimId, 'claim': claim, 'evidences': evidence, 'label': 2}
